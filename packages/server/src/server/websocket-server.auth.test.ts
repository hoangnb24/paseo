...
describe("direct WebSocket socket-peer authentication", () => {
  test.each([
    ...
    "0:0:0:0:0:0:0:1",
    "::ffff:127.0.0.1",
  ])("allows a normalized loopback TCP peer to omit the bearer: %s", (remoteAddress) => {
    expect(isDirectWebSocketRequestAuthorized(
  ...
  test("does not grant local authority from foreign-origin or forwarded request metadata", () => {
    expect(isDirectWebSocketRequestAuthorized(request({
      remoteAddress: "192.168.1.20",
      origin: "https://foreign.example",
      host: "localhost:6767",
      forwardedFor: "127.0.0.1",
    }), CORRECT_PASSWORD_HASH)).toBe(false);
    expect(isDirectWebSocketRequestAuthorized(request({
